...
import (
	...
	helloworld "bitbucket.org/russia/test/grpcsniffer/hello"
)
...
func (s *myStream) Reassembled(rs []tcpassembly.Reassembly) {
	...
	helloResp := &helloworld.HelloReply{}
	respDesc, _ := descriptor.ForMessage(helloResp)
	...
	var jsonReq, jsonResp []byte
	var err error

	jsonReq, err = pb2json.Unmarshal(msg, reqDesc, "HelloRequest")
	if err == nil {
		fmt.Println(string(jsonReq))
		goto ok
	}

	jsonResp, err = pb2json.Unmarshal(msg, respDesc, "HelloReply")
	if err == nil {
...
func (s *myStream) ReassemblyComplete() {
	...
	fmt.Println("ReassemblyComplete")
}
...
func main() {
	...
	for {
		select {
		case packet := <-packets:
			if packet.TransportLayer().TransportFlow().Dst().String() == "50051" {
				//log.Println("---")
				//log.Println(packet.TransportLayer().LayerContents())
				//log.Println("===")
			}
